...
try {
    switch ($action) {
        ...
        case 'generate_habit':
            ...
            if (empty($goal)) {
                echo json_encode(['success' => false, 'error' => 'Please describe your goal']);
                exit;
            }

            if (strlen($goal) > 200) {
                echo json_encode(['success' => false, 'error' => 'Goal too long (max 200 chars)']);
                exit;
            }
            ...
            if (!$rawResponse) {
                echo json_encode([
                    'success' => false,
                    'error' => 'AI is not available right now. Please add the habit manually.'
                ]);
                exit;
            }
            ...
            if (!$habitData || !isset($habitData['habitName'])) {
                echo json_encode([
                    'success' => false,
                    'error' => 'AI response was not valid. Try rephrasing your goal.',
                    'raw' => $rawResponse
                ]);
                exit;
            }
        ...
        case 'chat':
            ...
            if (strlen($userMessage) > 500) {
                echo json_encode(['success' => false, 'error' => 'Message too long (max 500 chars)']);
                exit;
            }
            ...
            echo json_encode([
                'success' => true,
                'reply' => $reply ?: "I'm having trouble thinking right now. Try again in a moment!"
            ]);
            break;
